...
- tryRelease(int)：独占方式。尝试释放资源，要求立即返回。成功则返回true，失败则返回false。
...
此方法主要用于检查状态，看看自己是否真的可以去休息了（进入waiting状态，如果线程状态转换不熟，可以参考本人上一篇写的[Thread详解](.\多线程.md)），万一队列前边的线程都放弃了只是瞎站着，那也说不定，对吧！

~~~java
private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
    int ws = pred.waitStatus;//拿到前驱的状态
    if (ws == Node.SIGNAL)
        //如果已经告诉前驱拿完号后通知自己一下——“通知状态”，那就可以安心休息了
        return true;
    if (ws > 0) {
        /*
